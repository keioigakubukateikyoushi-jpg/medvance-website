...
同様に確からしい標本点を数え、事象の確率を場合の数の比で求める。定義を言えるだけでなく、条件を読み、途中過程を示し、自分で誤りを検算できる状態を目標とする。このPartでは「確率の定義」だけを完結させ、別Partの概念を無理に詰め込まない。

## 受験での位置づけ

基礎問題では、用語の意味、条件の読み取り、標準手順の再現が得点の土台になる。複合問題でも、最初に「確率の定義」を正しく処理できなければ、その後の計算や考察が連鎖的に崩れる。答えだけでなく、どの条件を使い、なぜその操作を選んだかを説明できることが重要である。
...
「確率の定義」では、見た目が似た選択肢でも定義が要求する条件を満たすかどうかで結論が変わる。まず対象を確定し、使える情報と使えない情報を分ける。その後に操作へ進めば、標本空間を重複して数えるという典型誤答を防げる。
...
公平なさいころを1回投げ、偶数が出る確率を求める。

Ω={1,2,3,4,5,6}、A={2,4,6}である。

各目は同様に確からしいのでP(A)=3/6=1/2である。
