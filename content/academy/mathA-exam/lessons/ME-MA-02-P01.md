...
段階ごとの選択を樹形図で表し、漏れと重複なく数え上げる。定義を言えるだけでなく、条件を読み、途中過程を示し、自分で誤りを検算できる状態を目標とする。このPartでは「樹形図」だけを完結させ、別Partの概念を無理に詰め込まない。

## 受験での位置づけ

基礎問題では、用語の意味、条件の読み取り、標準手順の再現が得点の土台になる。複合問題でも、最初に「樹形図」を正しく処理できなければ、その後の計算や考察が連鎖的に崩れる。答えだけでなく、どの条件を使い、なぜその操作を選んだかを説明できることが重要である。
...
「樹形図」では、見た目が似た選択肢でも定義が要求する条件を満たすかどうかで結論が変わる。まず対象を確定し、使える情報と使えない情報を分ける。その後に操作へ進めば、各段階で同じ選択肢を使えるか確認しないという典型誤答を防げる。
...
1,2,3から異なる2個を順に並べる。最初の枝は1,2,3の3本である。

1の枝からは2,3、2の枝からは1,3、3の枝からは1,2へ伸ばす。

末端は12,13,21,23,31,32の6個であり、3×2=6とも一致する。
...
(1) 表裏のある硬貨を2回投げる結果を樹形図で列挙せよ。

(2) A,B,Cから異なる2文字を順に並べよ。

(3) 1,2,3から和が4になる2数の順序対を列挙せよ。
